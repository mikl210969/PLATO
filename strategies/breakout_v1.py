...
class BreakoutStrategyV1(AdaptiveStrategy):
    def __init__(self, config: Dict[str, Any], atr_value: float = 0.5, context_manager=None):
        ...
        self._fallback_params = {
            ...
            "min_eaten_pct": config.get('min_eaten_pct', 0.70),
            "min_wall_age_sec": config.get('min_wall_age_sec', 30),
            "max_wall_exposure_pct": config.get('max_wall_exposure_pct', 0.15),
            "min_wall_updates": config.get('min_wall_updates', 30),
            "max_wall_cv": config.get('max_wall_cv', 0.15),
            "bypass_filters": config.get('bypass_filters', False),
            "min_confidence": config.get('min_confidence', 0.6),
            "delta_spike_ratio": config.get('delta_spike_ratio', 3.0),
        }
        
        self._delta_history = deque(maxlen=10)
        self._price_history = deque(maxlen=10)
        
        print(f" [DEBUG INIT] BreakoutV1: force_test_signal={self._fallback_params['force_test_signal']}, min_eaten={self._fallback_params['min_eaten_pct']}")
    ...
    async def generate_signal(self, context: Dict[str, Any]) -> Optional[EnrichedSignal]:
        ...
        if params.get('force_test_signal') and (now - self._last_test_signal_time >= params.get('test_signal_interval', 60)):
            ...
            return EnrichedSignal(
                ...
                execution_params={"quantity": params['fixed_lot_size'], "sl_price": sl_price, "tp1_price": tp_price, "tp2_price": tp_price}
            )
        ...
        if not params.get('bypass_filters', False) and market_regime == 'FLAT':
            return None
        ...
        min_updates = params.get('min_wall_updates', 30)
        max_cv = params.get('max_wall_cv', 0.15)
        min_eaten = params.get('min_eaten_pct', 0.70)
        spike_ratio = params.get('delta_spike_ratio', 3.0)
        max_exposure = params.get('max_wall_exposure_pct', 0.15)
        fixed_lot = params.get('fixed_lot_size', 7.0)
        ...
        for wall in walls_ask:
            ...
            qty = min(fixed_lot, max_allowed_qty)
            if qty < 0.1:
                continue
            ...
            if conf > best_confidence:
                ...
                best_signal = {
                    ...
                    'tp_price': tp_price, 'qty': qty, 'edge_price': wall['price']
                }
        ...
        cooldown = params.get('cooldown_sec', 60)
