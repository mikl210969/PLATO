...
async def test_t15_partial_close_updates_passport(setup):
    ...
    # 🔥 Формируем client_order_id из РЕАЛЬНОГО passport_id
    event = MagicMock()
    event.source = "ws_adapter"
    event.payload = {
        "client_order_id": f"CLOSE_TP1_HIT_{passport.passport_id}",
        "executed_qty": 3.5,
        "avg_price": 96.81,
    }
...
async def test_t15b_full_close_marks_passport_closed(setup):
    ...
    event.source = "ws_adapter"
    event.payload = {
        "client_order_id": f"CLOSE_TP2_HIT_{passport.passport_id}",
        "executed_qty": 3.5,
        "avg_price": 96.56,
    }
